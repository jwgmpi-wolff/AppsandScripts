...
import { getCostByProject, getSparklineForProject, filterEvents, sumCost, sumTokens, formatCost, formatTokens, formatNumber } from '@/data/queries';
...
export function ProjectsPage() {
  ...
  const totalModelCost = useMemo(() => perModelData.reduce((s, d) => s + d.cost, 0), [perModelData]);

  const donutData = useMemo(() =>
    perModelData.map(d => ({
      label: getModelName(d.modelId),
      value: totalModelCost > 0 ? d.cost : d.tokens,
      color: getProviderColor(getModelProviderId(d.modelId)),
    })),
  [perModelData, totalModelCost]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projectCosts.map(({ project, cost, tokens }) => {
          ...
          const total30Tokens = sumTokens(filterEvents(live.usageEvents, live.models, {}));
          const pct = total30Cost > 0
            ? (cost / total30Cost * 100).toFixed(1)
            : total30Tokens > 0 ? (tokens / total30Tokens * 100).toFixed(1) : '0';
