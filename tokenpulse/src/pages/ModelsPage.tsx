...
export function ModelsPage() {
  ...
  const chartData = useMemo(() =>
    modelData.map(d => ({
      label: d.model.name,
      value: d.model.inputPricePer1k,
      color: getProviderColor(d.model.providerId),
    })),
  [modelData]);

  const chartDataOutput = useMemo(() =>
    modelData.map(d => ({
      label: d.model.name,
      value: d.model.outputPricePer1k,
      color: getProviderColor(d.model.providerId),
    })),
  [modelData]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <div className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Input Price per 1K Tokens</div>
          <HorizBarChart
            data={chartData}
        ...
        <Card>
          <div className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-4">Output Price per 1K Tokens</div>
          <HorizBarChart
            data={chartDataOutput}
            formatValue={v => `$${v.toFixed(5)}`}
          />
        </Card>
      </div>
